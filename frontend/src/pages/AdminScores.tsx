...
import type { Match, PaginatedResponse, Team } from "../types";
import { isKnockoutStage } from "../utils/predictions";
...
function sideLabel(
  ...
  if (team) return localizedTeam(team, language);
  return slot ?? "—";
}
...
export default function AdminScores() {
  ...
  const load = () => {
    setLoading(true);
    Promise.all([fetchAllMatches({}), api.get<Team[]>("/teams")])
      .then(([items, teamsRes]) => {
        setMatches(items);
        setTeams(teamsRes.data);
        const d: Record<number, Draft> = {};
        for (const m of items) {
          d[m.id] = draftFromMatch(m);
        }
        setDrafts(d);
      })
  ...
  const filtered = useMemo(() => {
    return matches.filter((m) => {
      ...
      const homeName = sideLabel(m.home_team, m.bracket_home_slot, i18n.language).toLowerCase();
      const awayName = sideLabel(m.away_team, m.bracket_away_slot, i18n.language).toLowerCase();
      return (
        ...
        m.bracket_home_slot?.toLowerCase().includes(q) ||
        m.bracket_away_slot?.toLowerCase().includes(q) ||
  ...
  const saveRow = async (m: Match) => {
    ...
    try {
      ...
      if (isKnockoutStage(m.stage)) {
        if (d.homeTeamId != null) payload.home_team_id = d.homeTeamId;
        if (d.awayTeamId != null) payload.away_team_id = d.awayTeamId;
        if (d.home === d.away && d.status === "completed") {
          payload.winner_team_id = d.winnerTeamId;
        }
      }

      const { data } = await api.patch<Match>(`/matches/${m.id}/score`, payload);
      setMatches((prev) => prev.map((x) => (x.id === m.id ? data : x)));
      setDrafts((prev) => ({
        ...prev,
        [m.id]: draftFromMatch(data),
      }));
  ...
  const teamOptionLabel = (team: Team) =>
    `${team.fifa_code} · ${localizedTeam(team, i18n.language)}`;
          ...
          <table className="w-full text-sm min-w-[52rem]">
            <thead>
              <tr className="text-left text-gray-500 border-b bg-gray-50">
                ...
                <th className="py-3 px-3 font-medium min-w-[220px]">{t("adminScores.match")}</th>
                <th className="py-3 px-3 font-medium">{t("adminScores.score")}</th>
                <th className="py-3 px-3 font-medium min-w-[140px]">{t("adminScores.winner")}</th>
            ...
            <tbody>
              {sorted.map((m) => {
                ...
                const showWinnerPicker = knockout && d.home === d.away;
                const homeDisplay = sideLabel(m.home_team, m.bracket_home_slot, i18n.language);
                const awayDisplay = sideLabel(m.away_team, m.bracket_away_slot, i18n.language);
                const winnerOptions = [d.homeTeamId, d.awayTeamId].filter(
                ...
                return (
                  <tr key={m.id} className="border-b last:border-0 hover:bg-gray-50/80">
                    ...
                    <td className="py-2.5 px-3 align-middle">
                      ...
                      {knockout && (!m.home_team || !m.away_team) && (
                        <div className="mt-2 flex flex-col gap-1.5">
                          {!m.home_team && (
                            <select
                              ...
                              onChange={(e) =>
                                updateDraft(m.id, {
                                  homeTeamId: e.target.value ? Number(e.target.value) : null,
                                })
                              }
                              className="border border-amber-300 bg-amber-50 rounded px-2 py-1 text-xs max-w-[220px]"
                            >
                              <option value="">{t("adminScores.assignHome")}</option>
                              {teams.map((team) => (
                          ...
                          {!m.away_team && (
                            <select
                              ...
                              onChange={(e) =>
                                updateDraft(m.id, {
                                  awayTeamId: e.target.value ? Number(e.target.value) : null,
                                })
                              }
                              className="border border-amber-300 bg-amber-50 rounded px-2 py-1 text-xs max-w-[220px]"
                            >
                              <option value="">{t("adminScores.assignAway")}</option>
                              {teams.map((team) => (
                                <option key={team.id} value={team.id}>
                                  {teamOptionLabel(team)}
                                </option>
                    ...
                    <td className="py-2.5 px-3 align-middle">
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          aria-label={t("adminScores.homeGoals")}
                        />
                        ...
                        <input
                          ...
                          className="w-14 text-center border border-gray-300 rounded px-1 py-1"
                          aria-label={t("adminScores.awayGoals")}
                        />
                      </div>
                      {knockout && (
                        <p className="text-[11px] text-gray-500 mt-1 max-w-[12rem]">
                          {t("adminScores.knockout90Hint")}
                        </p>
                      )}
                    </td>
                    <td className="py-2.5 px-3 align-middle">
                      {showWinnerPicker ? (
                        <select
                          value={d.winnerTeamId ?? ""}
                          onChange={(e) =>
                            updateDraft(m.id, {
                              winnerTeamId: e.target.value ? Number(e.target.value) : null,
                            })
                          }
                          className="border border-gray-300 rounded px-2 py-1 text-xs max-w-[180px]"
                        >
                          <option value="">{t("adminScores.selectWinner")}</option>
                          {winnerOptions.map((id) => {
                            const team = teamsById.get(id) ?? (m.home_team?.id === id ? m.home_team : m.away_team);
                            if (!team) return null;
                            return (
